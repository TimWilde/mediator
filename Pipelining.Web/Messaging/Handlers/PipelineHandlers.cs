...
public class PipelineHandlers
{
   ...
   public class BuildDaysSinceResponse: IPipelineRequestHandler<int, DaysSinceResponse>
   {
      private readonly ILogger<BuildDaysSinceResponse> logger;

      public BuildDaysSinceResponse( ILogger<BuildDaysSinceResponse> logger )
      {
         this.logger = logger;
      }

      public Task<DaysSinceResponse> Process( int input, CancellationToken cancellationToken )
      {
         logger.LogInformation( "-- Wrapping an int in a DaysSinceResponse" );
         return Task.FromResult( new DaysSinceResponse( input ) );
      }
   }
}
